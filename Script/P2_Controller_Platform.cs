...
//todo: ダッシュとスペシャルスキルの操作をキーボードにて行えるようにしている。本アプリでは消す

public class P2_Controller_Platform : MonoBehaviour
{
	...
	public GameObject player;  // Player2オブジェクトを参照
	...
	private void Start()
	{
		...
		//手のコライダーを取得
		handRCollider = GameObject.Find("hand2RCollide").GetComponent<SphereCollider>();
		handLCollider = GameObject.Find("hand2LCollide").GetComponent<SphereCollider>();
		...
		trackingCamera = GameObject.Find("Player2_Camera").GetComponent<Camera>();
		isThrow = false;
		isParth = false;
		...
		player1Controller = player.GetComponent<P_Controller_Platform>();

		Special = false;
	}

	private void setAnim(RuntimeAnimatorController w)
	{
		anim.runtimeAnimatorController = w;
	}
	...
	public GameObject GetSearchWeapon()
	{
		return wp;
	}
	...
	private void Update()
	{
		...
		if (canGrab && Input.GetKeyDown("joystick 2 button 2") && !isJump)
		{
		...
		if (currentWeapon != null && Input.GetKeyDown("joystick 2 button 0") && !isJump)
		{
			isAnimator = !isAnimator;
			Drop();
	...
	private void CheckWeapons()
	{
			...
			|| Physics.Raycast(obj.transform.position, -obj.transform.right, out hit, distance))
		{
			if ((hit.transform.CompareTag("Item") || hit.transform.CompareTag("Item2") ||
				hit.transform.CompareTag("Item3") || hit.transform.CompareTag("Item4") || hit.transform.CompareTag("Item5") || hit.transform.CompareTag("Item6")) && hit.transform.gameObject != player1Controller.GetWeapon())
			{
				isParth = true;
				canGrab = true;
	...
	void Rotate()
	{
		...
		float verticalInput = Input.GetAxis("Vertical2") * -1.0f;
	...
	void Attack()
	{
		...
		if (Input.GetKeyDown("joystick 2 button 1")/* || Input.GetKeyDown(KeyCode.O)*/)
		{
			isAttack = true;
	...
	void Dodge()
	{
		float lTriInput = Input.GetAxis("LTrigger2");
		...
		if (lTriInput >= 0.6f || Input.GetKeyDown(KeyCode.LeftControl)) //todo:シフト入力消す
		{
	...
	void Block()
	{
		...
		if (Input.GetMouseButtonDown(1) || Input.GetKeyDown("joystick 2 button 4"))
		{
		...
		if (Input.GetMouseButtonUp(1) || Input.GetKeyUp("joystick 2 button 4"))
		{
	...
	void Crouch()
	{
		if (/*Input.GetKeyDown(KeyCode.LeftControl) || */Input.GetKeyDown("joystick 2 button 8"))
		{
			isCrouch = true;
			anim.SetBool("Crouch", true);
		}
		if (/*Input.GetKeyUp(KeyCode.LeftControl) || */Input.GetKeyUp("joystick 2 button 8"))
		{
			isCrouch = false;
	...
	void Jump()
	{
		if (Input.GetKeyUp("joystick 2 button 3") && isGround)
		{
			anim.SetBool("Block", false);
			anim.SetBool("Crouch", false);
	...
	void Skill1()
	{
		float rTriInput = Input.GetAxis("RTrigger2");
		...
		if (specialWave.active == true)
		{
			...
			if (rTriInput >= 0.6f || Input.GetKeyDown(KeyCode.LeftShift)) //todo:シフト入力消す
			{
				Skill6();
	...
	void Skill3()
	{
		if (Input.GetKeyUp("joystick 2 button 5") && !isAttack)
		{
	...
	void OnTriggerEnter(Collider other)
	{
		...
		if (other.gameObject.CompareTag("Special_P1"))
		{
			Skill8();
		...
		if (other.gameObject.CompareTag("Hand_P1"))
		{
			hitPoint -= punch;
			hpBar.Damage(punch);
			Skill7();
		...
		if (player1Controller.isThrow && other.gameObject == player1Controller.GetThrowWeapon()
			...
			|| other.gameObject.CompareTag("Item4") || other.gameObject.CompareTag("Item5") || other.gameObject.CompareTag("Item6")))
		{
			...
			hpBar.Damage(giveDamage); // 仮にダメージ量をcarに設定しています
									  ...
									  //Debug.Log("接触");
			player1Controller.isThrow = false;
		}
	}
